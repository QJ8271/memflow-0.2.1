...
impl<'a, T: Into<umem> + Copy> ops::Add<&'a T> for Address {
    ...
    fn add(self, other: &'a T) -> Self {
        Self(self.0.wrapping_add((*other).into()))
    }
}
...
impl<'a, T: Into<umem> + Copy> ops::Sub<&'a T> for Address {
    ...
    fn sub(self, other: &'a T) -> Self {
        Self(self.0.wrapping_sub((*other).into()))
    }
}

/// Subtracts a `Address` from a `Address` resulting in a `imem`.
///
/// # Examples
///
/// ```
...
impl ops::Sub for Address {
    ...
    fn sub(self, other: Self) -> imem {
        if self.0 > other.0 {
            (self.0.wrapping_sub(other.0)) as imem
        } else {
            -((other.0.wrapping_sub(self.0)) as imem)
        }
    }
}
